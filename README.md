...
```console
docker run cisagov/pshtt_reporter:1.4.6
```
...
1. Create a `compose.yml` file similar to the one below to use [Docker Compose](https://docs.docker.com/compose/).
    ...
    services:
      pshtt_reporter:
        image: cisagov/pshtt_reporter:1.4.6
        volumes:
          - type: bind
            source: <your_log_dir>
...
1. Pull the new image:
    ...
    ```console
    docker pull cisagov/pshtt_reporter:1.4.6
    ```
...
| --------- | ----------- |
| `cisagov/pshtt_reporter:1.4.6` | An exact release version. |
...
```console
docker build \
  --tag cisagov/pshtt_reporter:1.4.6 \
