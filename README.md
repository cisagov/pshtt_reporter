...
```console
docker run cisagov/pshtt_reporter:1.2.4
```
...
1. Create a `docker-compose.yml` file similar to the one below to use [Docker Compose](https://docs.docker.com/compose/).
    ...
    services:
      pshtt_reporter:
        image: cisagov/pshtt_reporter:1.2.4
        volumes:
          - type: bind
            source: <your_log_dir>
...
1. Pull the new image:
    ...
    ```console
    docker pull cisagov/pshtt_reporter:1.2.4
    ```
...
|-----------|-------------|
|`cisagov/pshtt_reporter:1.2.4`| An exact release version. |
...
```console
docker build \
  --tag cisagov/pshtt_reporter:1.2.4 \
