...
def main():
    """Generate all reports."""
    with open(f"{SHARED_DATA_DIR}/artifacts/unique-agencies.csv") as agency_csv:
        for row in sorted(csv.reader(agency_csv)):
            bash_command = [f"{HOME_DIR}/report/generate_https_scan_report.py", row[0]]
            ...
            # nosec.
            subprocess.run(bash_command)  # nosec B404, B603
