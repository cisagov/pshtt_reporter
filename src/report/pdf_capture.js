...
if (system.args.length != 5) {
  ...
  page.paperSize = {
    width: vp_width + "px",
    height: vp_height + 12 + "px",
    margin: "0px",
  }; // Add 12 pixels of height to keep PDF on 1 page
    ...
    function checkReadyState() {
      setTimeout(function () {
        var readyState = page.evaluate(function () {
          return (
            typeof cybex_chart4 !== "undefined" &&
            cybex_chart4.data().length > 0
          );
        });
        ...
        if (readyState) {
          setTimeout(function () {
            onPageReady();
          }, 1000); // Wait for d3 transition to complete after data has been rec'd
        } else {
          checkReadyState();
        }
      }, 200);
